...
About, `offerings/index.html`, the four offering pages and `contact.html` are
currently **Coming Soon splash pages**: `.page-header` with a `.soon-badge`, then
...
`contact.html` is the exception, because its badge says something different
("Booking and forms coming soon") and its card is headed "Email reaches us today."

Every splash points at `/contact.html` except Contact itself, which cannot link
to itself and carries a `mailto:` instead. That mailto is the site's only working
conversion path while these pages are down, so do not remove it.

**The full pages are not lost.** Their last complete versions are in git at commit
`0b5f3a6`. Restore one with
`git show 0b5f3a6:about.html > about.html`, then regenerate its `.md` twin and
